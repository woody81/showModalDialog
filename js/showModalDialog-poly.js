(function() {
    ...
    window.showModalDialog = window.showModalDialog || function(url, arg, opt) {
        ...
        if(caller.indexOf('yield') >= 0 || caller.indexOf('await') >= 0) {
            promise().done(function() {
                return new Promise(function(resolve, reject) {
                    var cancelButton = content.getElementById('cancel-button');
                    cancelButton.addEventListener('close', function() {
        ...
        document.getElementById('cancel-button').addEventListener('close', function() {
            var returnValue = document.getElementById('dialog-body').contentWindow.returnValue;
            document.body.removeChild(dialog);
